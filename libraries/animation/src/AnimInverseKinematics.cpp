...
const AnimPoseVec& AnimInverseKinematics::overlay(const AnimVariantMap& animVars, float dt, Triggers& triggersOut, const AnimPoseVec& underPoses) {
    ...
    if (_relativePoses.size() != underPoses.size()) {
        ...
        PROFILE_RANGE_EX("ik", "ik/relax", 0xffff00ff, 0);
    ...
    if (!_relativePoses.empty()) {
        ...
        {
            PROFILE_RANGE_EX("ik", "ik/computeTargets", 0xffff00ff, 0);
        ...
        if (targets.empty()) {
            ...
            {
                PROFILE_RANGE_EX("ik", "ik/shiftHips", 0xffff00ff, 0);
            ...
            {
                PROFILE_RANGE_EX("ik", "ik/ccd", 0xffff00ff, 0);
                solveWithCyclicCoordinateDescent(targets);
            }

            {
                PROFILE_RANGE_EX("ik", "ik/measureHipsOffset", 0xffff00ff, 0);

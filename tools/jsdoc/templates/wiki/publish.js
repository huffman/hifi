...
exports.publish = function(data, opts) {
    ...
    const WIKI_API = "https://wiki.highfidelity.com/api.php";
    ...
    waterfall([
            function(callback) {
                request({
                    ...
                    callback(null, body.query.tokens.logintoken);
                });
            },
            function(token, callback) {
                request({
                    ...
                    console.log(error, body);
                    cookies = `mediawikiUserName=Huffman; mediawiki_session=${body.login.sessionid};`;
            ...
            function(callback) {
                request({
                    ...
                    console.log(error, body);
                    callback(null, body.query.tokens.csrftoken);
                });
            },
            function(token, callback) {
                request({
                    ...
                    console.log('edit request', error, body);
                    callback(null);
                });
            }
    ], function(err) {
    ...
    //         json: true
    //     }, function(error, response, body) {
    //         console.log(error, body, body.login.token);
    //         request({
    //             url: WIKI_API,
    //             method: 'POST',
    //             qs: {
    ...
    if (opts.destination === 'console') {
        if (opts.query && opts.query.format === 'xml') {
            ...
            root.namespaces.forEach(function(ns) {
                console.log(ns.name);
                ns.functions.forEach(function(fn) {
                    //console.log(`\t${ns.name}.${fn.name}`);
                    var signature = `${ns.name}.${fn.name}(`;
                    var params = []
